...
class ConversationOrchestrator(BaseAgent):
    ...
    def __init__(self, llm_manager, logger=None):
        ...
        if self.logger:
            self.logger.log_agent_work("ORCHESTRATOR_AGENT", "初始化完成", "对话协调器已就绪（无监控模式）")
    ...
    def start_conversation(self, conversation_id: str, problem_content: str, user_id: str = None):
        """启动对话"""
        if self.logger:
            self.logger.log_agent_work("ORCHESTRATOR_AGENT", "启动对话", f"对话ID: {conversation_id}（无监控模式）")
...
def main():
    ...
    # monitor_agent 被移除
    knowledge_state_agent = KnowledgeStateAgent("knowledge_state", llm_manager, logger)
    ...
    # 注册智能体到事件总线（注意：不注册monitor_agent）
    event_bus.register_agent(teacher_agent)
    event_bus.register_agent(student_agent)
    # event_bus.register_agent(monitor_agent)  # 被移除
    event_bus.register_agent(knowledge_state_agent)
    ...
    # monitor_agent.set_event_bus(event_bus)  # 被移除
    knowledge_state_agent.set_event_bus(event_bus)
    ...
    # monitor_agent.initialize()  # 被移除
    knowledge_state_agent.initialize()
    ...
    teacher_agent.start()
    student_agent.start()
    # monitor_agent.start()  # 被移除
    knowledge_state_agent.start()
    ...
    output_dir = os.path.join("run_output", f"output_wo_mod_{timestamp}")
