...
"""
消融实验2: w/o.Mod 变体模型
移除对话监管智能体，教师智能体独立生成响应，不进行多维度质量审核
保留完整的ICECoT思维链逻辑
"""
...
5. 响应生成：生成针对性的教学回复"""
        ...
        if self.logger:
            self.logger.log_agent_work("TEACHER", "初始化完成", "无监控ICECoT教师智能体已就绪")

    def initialize(self):
        """初始化教师智能体"""
        self.update_state("ready", True)
        self.update_state("icecot_enabled", True)
        self.update_state("monitor_disabled", True)
    ...
    def _handle_student_message(self, message: Message):
        ...
        if teacher_response:
            # 直接发送给学生，无监控审核
            self._send_response_directly(teacher_response, round_number)
    ...
    def _handle_knowledge_analysis(self, message: Message):
        ...
        if self.pending_student_message:
            ...
            if teacher_response:
                self._send_response_directly(teacher_response, student_data["round_number"])
    ...
    def _execute_icecot_pipeline_with_knowledge(self, student_message: str, student_state: Dict[str, Any], round_number: int) -> str:
        ...
        if self.logger:
            self.logger.log_agent_work("TEACHER", "开始ICECoT流程（无监控）", f"第{round_number}轮分析")
        ...
        if self.logger:
            self.logger.log_agent_work("TEACHER", "ICECoT流程完成（无监控）", f"生成回复长度: {len(teacher_response)}字符")
...
"""
    ...
    def _generate_teaching_response_with_knowledge(self, student_message: str, emotion_analysis: Dict[str, Any], 
        ...
        if self.logger:
            ...
            self.logger.log_agent_work("TEACHER", "开始生成回复（无监控）", f"策略: {strategy}")
...
请生成相应的采用苏格拉底式教学方法的针对性教学回复。"""
        ...
        if self.logger:
            if response:
                self.logger.log_agent_work("TEACHER", "教学回复生成成功（无监控）", f"回复长度: {len(result)}字符")
            else:
                self.logger.log_agent_work("TEACHER", "教学回复生成失败", "使用默认回复")
        
        return result

    def _send_response_directly(self, teacher_response: str, round_number: int):
        """直接发送回复给学生（无监控审核）"""
        # 添加到对话历史
        self.conversation_history.append({
            "role": "teacher",
        ...
        if self.logger:
            self.logger.log_agent_work("TEACHER", "回复已直接发送", f"第{round_number}轮，长度: {len(teacher_response)}字符")