...
def evaluate_method_dataset(method_name: str, data_files: List[str], 
                          dimensions: List[str], llm_manager: SimpleLLMManager) -> Dict[str, Any]:
    """评估一个方法的所有数据集"""
    print(f"\n{'='*60}")
    print(f"开始评估方法: {method_name}")
    print(f"{'='*60}")
...
def save_evaluation_results(results: List[Dict[str, Any]], output_file: str):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        summary_df.to_excel(writer, sheet_name='评估汇总', index=False)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="评估不同方法生成的教师回复质量")
    parser.add_argument("--data_dir", type=str, required=True, help="包含对话数据文件的目录路径")
    parser.add_argument("--output_file", type=str, default="teacher_response_evaluation_results.xlsx", help="输出评估结果文件路径")
    ...
    data_files = []
    # 查找对比实验的8种方法生成的数据
    for method in ["ToT", "Self_Consistency", "Best_of_N", "Zero_shot", "ICL", "CoT", "ICL_CoT", "Socratic_Induction"]:
        method_file = data_dir / f"{method}_conversations.json"
        if method_file.exists():
            data_files.append((method, method_file))
    
    # 查找原系统的数据文件
    original_files = list(data_dir.glob("*original*conversations.json")) + list(data_dir.glob("*our_system*conversations.json"))
    for file in original_files:
        data_files.append(("Original_System", file))
    
    if not data_files:
        print(f"错误: 在目录 {data_dir} 中没有找到对话数据文件")
        print("请确保目录中包含以下格式的文件：")
        print("  - ToT_conversations.json")
        print("  - Self_Consistency_conversations.json")
        print("  - Best_of_N_conversations.json")
        print("  - Zero_shot_conversations.json")
        print("  - ICL_conversations.json")
        print("  - CoT_conversations.json")
        print("  - ICL_CoT_conversations.json")
        print("  - Socratic_Induction_conversations.json")
        print("  - 原系统对话数据文件")
        sys.exit(1)
    ...
    for method_name, file_path in data_files:
        try:
            result = evaluate_method_dataset(method_name, [str(file_path)], dimensions, llm_manager)
            all_results.append(result)
        except Exception as e:
            print(f"方法 {method_name} 评估失败: {e}")
            # 创建失败结果
            failed_result = {
                "method_name": method_name,
                "total_conversations": 0,
